...
XS_INLINE constexpr UInt128::UInt128(const Int128 other) noexcept
    : high(static_cast<uint64>(other.high))
    , low(other.low)
{}

XS_INLINE constexpr UInt128::UInt128(const uint64 hi, const uint64 lo) noexcept
    : high(hi)
    , low(lo)
{}
...
XS_INLINE constexpr UInt128 UInt128::operator&(T other) const noexcept
{
...
template<typename T, typename>
XS_INLINE constexpr UInt128& UInt128::operator&=(T other) noexcept
{
    high = 0_ui64;
    low &= static_cast<uint64>(other);
    return *this;
...
template<typename T, typename>
XS_INLINE UInt128 UInt128::operator<<(T other) const noexcept
{
...
#else
    return (other < 64) ? UInt128((high << static_cast<uint8>(other)) + (low >> (64_ui8 - static_cast<uint8>(other))),
...
template<typename T, typename>
XS_INLINE UInt128& UInt128::operator<<=(T other) noexcept
{
    *this = *this << other;
    return *this;
}
...
#else
    return (other < 64) ? UInt128(high >> static_cast<uint8>(other),
...
template<typename T, typename>
XS_INLINE constexpr bool UInt128::operator==(T other) const noexcept
{
