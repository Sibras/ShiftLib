...
public:
    ...
    XS_INLINE void store(const SIMD12<T, Width>& other) noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            _mm_storeu_si32(&value8, _mm_srli_si128(other.values, 8 * sizeof(T)));
            ...
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                ...
                _mm_storel_epi64(&value8, _mm256_extracti128_si128(other.values1, 1));
            } else {
                _mm_storeu_si128(&value0, other.values0);
                _mm_storel_epi64(&value8, other.values1);
            }
        } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                _mm512_mask_storeu_epi32(&value0, _cvtu32_mask16(0x0FFF), other.values);
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                _mm256_store_epi32(&value0, other.values0);
                _mm_store_epi32(&value8, other.values1);
            } else {
                _mm_store_epi32(&value0, other.values0);
                _mm_store_epi32(&value4, other.values1);
                _mm_store_epi32(&value8, other.values2);
            }
        } else
#endif
        {
    ...
    XS_INLINE SIMD12<T, Width> load() const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            return SIMD12<T, Width>(_mm_loadu_si128(&value0));
        } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12<T, Width>(_mm256_loadu_si256(&value0));
            } else {
                return SIMD12<T, Width>(_mm_loadu_si128(&value0), _mm_loadu_si128(&value3));
            }
        } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12<T, Width>(_mm256_loadu_si256(&value0), _mm_load_si128(&value8));
            } else {
                return SIMD12<T, Width>(_mm_load_si128(&value0), _mm_load_si128(&value4), _mm_load_si128(&value8));
            }
        } else
#endif
        {
...
public:
    ...
    T value4, value5, value6, value7;
    T value8, value9, value10, value11;
    NoExport::SIMDPad<T2, sizeof(T) * 4> pad;
    ...
    XS_INLINE void store(const SIMD12<T, Width>& other) noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                _mm_store_si128(&value8, other.values1);
            } else {
                _mm_store_si128(&value0, other.values0);
                _mm_store_si128(&value4, other.values1);
                _mm_store_si128(&value8, other.values2);
            }
        } else
#endif
        {
    ...
    XS_INLINE SIMD12<T, Width> load() const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            return SIMD12<T, Width>(_mm_load_si128(&value0));
        } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12<T, Width>(_mm256_load_si256(&value0));
            } else {
                return SIMD12<T, Width>(_mm_load_si128(&value0), _mm_load_si128(&value3));
            }
        } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12<T, Width>(_mm256_load_si256(&value0), _mm_load_si128(&value8));
            } else {
                return SIMD12<T, Width>(_mm_load_si128(&value0), _mm_load_si128(&value4), _mm_load_si128(&value8));
            }
        } else
...
public:
    ...
    using DataPad = SIMD12DataPad<T, T2>;
    ...
    public:
        ...
        XS_INLINE Shuffle(const uint32 shuffle0, const uint32 shuffle1, const uint32 shuffle2, const uint32 shuffle3,
            const uint32 shuffle4, const uint32 shuffle5, const uint32 shuffle6, const uint32 shuffle7,
    ...
    XS_INLINE explicit SIMD12(const SIMD12<T, Width2>& other) noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (hasSIMD<T> && (Width > SIMDWidth::Scalar) && (Width2 > SIMDWidth::Scalar)) {
            ...
            if constexpr (isSame<T, float32> && hasSIMD256<T> && (Width2 >= SIMDWidth::B64)) {
                ...
                this->values6 = _mm_cvtss_f32(_mm_shuffle3232_ps(hi));
                this->values7 = _mm_cvtss_f32(_mm_shuffle3333_ps(hi));
                this->values8 = _mm_cvtss_f32(other.values1);
                this->values9 = _mm_cvtss_f32(_mm_shuffle3311_ps(other.values1));
                this->values10 = _mm_cvtss_f32(_mm_shuffle3232_ps(other.values1));
                this->values11 = _mm_cvtss_f32(_mm_shuffle3333_ps(other.values1));
                ...
                this->values5 = _mm_cvtss_f32(_mm_shuffle3311_ps(other.values1));
                this->values6 = _mm_cvtss_f32(_mm_shuffle3232_ps(other.values1));
                this->values7 = _mm_cvtss_f32(_mm_shuffle3333_ps(other.values1));
                this->values8 = _mm_cvtss_f32(other.values2);
                this->values9 = _mm_cvtss_f32(_mm_shuffle3311_ps(other.values2));
                this->values10 = _mm_cvtss_f32(_mm_shuffle3232_ps(other.values2));
                this->values11 = _mm_cvtss_f32(_mm_shuffle3333_ps(other.values2));
            }
        } else
#endif
        {
     ...
     */
    XS_INLINE SIMD12(const T value0, const T value1, const T value2, const T value3, const T value4, const T value5,
        const T value6, const T value7, const T value8, const T value9, const T value10, const T value11) noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                this->values = _mm512_set_ps(0, 0, 0, 0, static_cast<T>(value11), static_cast<T>(value10),
                    static_cast<T>(value9), static_cast<T>(value8), static_cast<T>(value7), static_cast<T>(value6),
                ...
                this->values0 = _mm256_set_ps(static_cast<T>(value7), static_cast<T>(value6), static_cast<T>(value5),
                ...
                this->values1 = _mm_set_ps(
                    static_cast<T>(value11), static_cast<T>(value10), static_cast<T>(value9), static_cast<T>(value8));
            } else {
                ...
                this->values2 = _mm_set_ps(
                    static_cast<T>(value11), static_cast<T>(value10), static_cast<T>(value9), static_cast<T>(value8));
            }
        } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                this->values = _mm512_set_epi32(0, 0, 0, 0, static_cast<T>(value11), static_cast<T>(value10),
                    static_cast<T>(value9), static_cast<T>(value8), static_cast<T>(value7), static_cast<T>(value6),
                    static_cast<T>(value5), static_cast<T>(value4), static_cast<T>(value3), static_cast<T>(value2),
                    static_cast<T>(value1), static_cast<T>(value0));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                this->values0 = _mm256_set_epi32(static_cast<T>(value7), static_cast<T>(value6), static_cast<T>(value5),
                    static_cast<T>(value4), static_cast<T>(value3), static_cast<T>(value2), static_cast<T>(value1),
                    static_cast<T>(value0));
                ...
                this->values1 = _mm_set_epi32(
                    static_cast<T>(value7), static_cast<T>(value6), static_cast<T>(value5), static_cast<T>(value4));
                this->values2 = _mm_set_epi32(
                    static_cast<T>(value11), static_cast<T>(value10), static_cast<T>(value9), static_cast<T>(value8));
            }
        } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                this->values = _mm256_set_epi16(0, 0, 0, 0, static_cast<T>(value11), static_cast<T>(value10),
                    static_cast<T>(value9), static_cast<T>(value8), static_cast<T>(value7), static_cast<T>(value6),
                    static_cast<T>(value5), static_cast<T>(value4), static_cast<T>(value3), static_cast<T>(value2),
                    static_cast<T>(value1), static_cast<T>(value0));
            } else {
                this->values0 = _mm_set_epi16(static_cast<T>(value7), static_cast<T>(value6), static_cast<T>(value5),
                    static_cast<T>(value4), static_cast<T>(value3), static_cast<T>(value2), static_cast<T>(value1),
                    static_cast<T>(value0));
                this->values1 = _mm_set_epi16(0, 0, 0, 0, static_cast<T>(value11), static_cast<T>(value10),
                    static_cast<T>(value9), static_cast<T>(value8));
            }
        } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            this->values = _mm_set_epi8(0, 0, 0, 0, static_cast<T>(value11), static_cast<T>(value10),
                static_cast<T>(value9), static_cast<T>(value8), static_cast<T>(value7), static_cast<T>(value6),
                static_cast<T>(value5), static_cast<T>(value4), static_cast<T>(value3), static_cast<T>(value2),
                static_cast<T>(value1), static_cast<T>(value0));
        } else
     ...
     */
    XS_INLINE explicit SIMD12(const T value) noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                this->values1 = _mm256_castsi256_si128(this->values0);
            } else {
     ...
     */
    XS_INLINE explicit SIMD12(const BaseDef other) noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
     ...
     * @param other Value to set all elements to.
     */
    XS_INLINE explicit SIMD12(const InBaseDef other) noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
    ...
    XS_INLINE static SIMD12 Zero() noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (isSame<T, float32> && hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_setzero_si512());
            } else if constexpr (isSame<T, float32> && hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                const __m256i value = _mm256_setzero_si256();
                return SIMD12(value, _mm256_castsi256_si128(value));
            } else {
            ...
            if constexpr (isSame<T, float32> && hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_setzero_si256());
            } else {
                const __m128i value = _mm_setzero_si128();
                return SIMD12(value, value);
            }
        } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_setzero_si128());
        } else
#endif
        {
            return SIMD12(T{0});
    ...
    XS_INLINE static SIMD12 One() noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_set1_epi32(1.0));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            ...
            if constexpr (isSame<T, float32> && hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_set1_epi32(1.0));
            } else {
                const __m128i value = _mm_set1_epi32(1.0);
                return SIMD12(value, value);
            }
        } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_set1_epi8(1.0));
        } else
#endif
        {
            return SIMD12(T{1});
     ...
     */
    template<uint32 Index>
    XS_INLINE SIMD12& setValue(const BaseDef other) noexcept
    {
        static_assert(Index < 12, "Invalid Index: Index must be <12");
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
     ...
     */
    template<uint32 Index>
    XS_INLINE SIMD12& setValue(const InBaseDef other) noexcept
    {
        static_assert(Index < 12, "Invalid Index: Index must be <12");
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
     ...
     */
    template<uint32 Index>
    XS_INLINE SIMD12& setValue2(const SIMD2Def& other) noexcept
    {
        static_assert(Index < 6, "Invalid Index: Index must be <6");
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
     ...
     */
    template<uint32 Index>
    XS_INLINE SIMD12& setValue3(const SIMD3Def& other) noexcept
    {
        static_assert(Index < 4, "Invalid Index: Index must be <4");
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
     ...
     */
    template<uint32 Index>
    XS_INLINE SIMD12& setValue4(const SIMD4Def& other) noexcept
    {
        static_assert(Index < 3, "Invalid Index: Index must be <3");
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
     ...
     */
    template<uint32 Index>
    XS_INLINE SIMD12& setValue6(const SIMD6Def& other) noexcept
    {
        static_assert(Index < 2, "Invalid Index: Index must be <2");
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
     ...
     */
    template<uint32 Index0, uint32 Index1>
    XS_INLINE SIMD12& setValue4x2(const SIMD8Def& other) noexcept
    {
        static_assert(Index0 < 3 && Index1 < 3, "Invalid Index: Indexes must be <3");
    ...
    XS_INLINE SIMD12 mad(const SIMD12& other1, const SIMD12& other2) const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_add_epi32(_mm512_mullo_epi32(this->values, other1.values), other2.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_add_epi32(_mm256_mullo_epi32(this->values0, other1.values0), other2.values0),
                    _mm_add_epi32(_mm_mullo_epi32(this->values1, other1.values1), other2.values1));
            } else {
                return SIMD12(_mm_add_epi32(_mm_mullo_epi32(this->values0, other1.values0), other2.values0),
                    _mm_add_epi32(_mm_mullo_epi32(this->values1, other1.values1), other2.values1),
                    _mm_add_epi32(_mm_mullo_epi32(this->values2, other1.values2), other2.values2));
            }
        } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_add_epi16(_mm256_mullo_epi16(this->values, other1.values), other2.values));
            } else {
                return SIMD12(_mm_add_epi16(_mm_mullo_epi16(this->values0, other1.values0), other2.values0),
            ...
            if constexpr (hasSIMD256<T>) {
                ...
                return SIMD12(_mm_packus_epi16(_mm256_castsi256_si128(ret), _mm256_extracti128_si256(ret, 1)));
            } else {
                const __m128i ret0 =
                ...
                return SIMD12(_mm_packus_epi16(ret0, ret1));
            }
            ...
            if constexpr (hasSIMD256<T>) {
                ...
                return SIMD12(_mm_packs_epi16(_mm256_castsi256_si128(ret), _mm256_extracti128_si256(ret, 1)));
            } else {
                const __m128i ret0 =
                ...
                return SIMD12(_mm_packs_epi16(ret0, ret1));
            }
        } else
#endif
        {
    ...
    XS_INLINE SIMD12 mad(BaseDef other1, const SIMD12& other2) const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                return SIMD12(_mm256_add_epi32(_mm256_mullo_epi32(this->values0, other1.values), other2.values0),
                    _mm_add_epi32(
                        _mm_mullo_epi32(this->values1, _mm256_castsi256_si128(other1.values)), other2.values1));
            } else {
                return SIMD12(_mm_add_epi32(_mm_mullo_epi32(this->values0, other1.values), other2.values0),
            ...
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                ...
                return SIMD12(_mm_add_epi16(_mm_mullo_epi16(this->values0, other1.values), other2.values0),
            ...
            if constexpr (hasSIMD256<T>) {
                ...
                const __m128i ret1 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepu8_epi16(_mm_shuffle3322_epi32(this->values)), val),
                        _mm_cvtepu8_epi16(_mm_shuffle3322_epi32(other2.values)));
                return SIMD12(_mm_packus_epi16(ret0, ret1));
            }
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T>) {
                ...
                const __m128i ret1 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepi8_epi16(_mm_shuffle3322_epi32(this->values)), val),
                        _mm_cvtepi8_epi16(_mm_shuffle3322_epi32(other2.values)));
                return SIMD12(_mm_packs_epi16(ret0, ret1));
            }
        } else
#endif
     ...
     */
    template<bool EvenIfNotFree = true>
    XS_INLINE SIMD12 mad(const SIMD12& other1, const BaseDef other2) const noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                return SIMD12(_mm256_add_epi32(_mm256_mullo_epi32(this->values0, other1.values0), other2.values),
                    _mm_add_epi32(
                        _mm_mullo_epi32(this->values1, other1.values1), _mm256_castsi256_si128(other2.values)));
            } else {
                return SIMD12(_mm_add_epi32(_mm_mullo_epi32(this->values0, other1.values0), other2.values),
            ...
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                ...
                return SIMD12(_mm_add_epi16(_mm_mullo_epi16(this->values0, other1.values0), other2.values),
            ...
            if constexpr (hasSIMD256<T>) {
                ...
                const __m128i val = _mm_cvtepu8_epi16(other2.values);
                const __m128i ret0 = _mm_add_epi16(
                    _mm_mullo_epi16(_mm_cvtepu8_epi16(this->values), _mm_cvtepu8_epi16(other1.values)), val);
                const __m128i ret1 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepu8_epi16(_mm_shuffle3322_epi32(this->values)),
                                      _mm_cvtepu8_epi16(_mm_shuffle3322_epi32(other1.values))),
                        val);
                return SIMD12(_mm_packus_epi16(ret0, ret1));
            }
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T>) {
                ...
                const __m128i val = _mm_cvtepi8_epi16(other2.values);
                const __m128i ret0 = _mm_add_epi16(
                    _mm_mullo_epi16(_mm_cvtepi8_epi16(this->values), _mm_cvtepi8_epi16(other1.values)), val);
                const __m128i ret1 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepi8_epi16(_mm_shuffle3322_epi32(this->values)),
                                      _mm_cvtepi8_epi16(_mm_shuffle3322_epi32(other1.values))),
                        val);
                return SIMD12(_mm_packs_epi16(ret0, ret1));
            }
        } else
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                    ...
                    return SIMD12(_mm512_add_ps(
                        _mm512_mul_ps(this->values, _mm512_broadcastf128_ps(other1.values)), other2.values));
                }
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                    return SIMD12(
                        _mm256_fmadd_ps(this->values0, _mm256_broadcastf128_ps(other1.values), other2.values0),
                        _mm_fmadd_ps(this->values1, other1.values, other2.values1));
                } else {
                    return SIMD12(_mm256_add_ps(_mm256_mul_ps(this->values0, _mm256_broadcastf128_ps(other1.values)),
                                      other2.values0),
                        _mm_add_ps(_mm_mul_ps(this->values1, other1.values), other2.values1));
                }
            } else {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_add_epi32(
                    _mm512_mullo_epi32(this->values, _mm512_broadcasti128(other1.values)), other2.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_add_epi32(_mm256_mullo_epi32(this->values0, _mm256_broadcasti128(other1.values)),
                                  other2.values0),
                    _mm_add_epi32(_mm_mullo_epi32(this->values1, other1.values), other2.values1));
            } else {
                return SIMD12(_mm_add_epi32(_mm_mullo_epi32(this->values0, other1.values), other2.values0),
            ...
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_add_epi16(
                    _mm256_mullo_epi16(this->values, _mm256_broadcasti64(other1.values)), other2.values));
            } else {
                return SIMD12(
                    _mm_add_epi16(_mm_mullo_epi16(this->values0, _mm_shuffle00_epi64(other1.values0)), other2.values0),
                    _mm_add_epi16(_mm_mullo_epi16(this->values1, other1.values1), other2.values1));
            }
        } else if constexpr (isSame<T, uint8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T>) {
                ...
                const __m128i val = _mm_cvtepu8_epi16(other1.values);
                const __m128i ret0 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepu8_epi16(this->values), _mm_shuffle00_epi64(val)),
                        _mm_cvtepu8_epi16(other2.values));
                const __m128i ret1 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepu8_epi16(_mm_shuffle3322_epi32(this->values)), val),
                        _mm_cvtepu8_epi16(_mm_shuffle3322_epi32(other2.values)));
            ...
            if constexpr (hasSIMD256<T>) {
                const __m256i ret = _mm256_add_epi16(_mm256_mullo_epi16(_mm256_cvtepi8_epi16(this->values),
                                                         _mm256_broadcasti64(_mm_cvtepi8_epi16(other1.values))),
                    _mm256_cvtepi8_epi16(other2.values));
                return SIMD12(_mm_packs_epi16(_mm256_castsi256_si128(ret), _mm256_extracti128_si256(ret, 1)));
            } else {
                const __m128i val = _mm_cvtepi8_epi16(other1.values);
                const __m128i ret0 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepi8_epi16(this->values), _mm_shuffle00_epi64(val)),
                        _mm_cvtepi8_epi16(other2.values));
                const __m128i ret1 =
                    _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepi8_epi16(_mm_shuffle3322_epi32(this->values)), val),
                        _mm_cvtepi8_epi16(_mm_shuffle3322_epi32(other2.values)));
...
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            const __m128 val0A = _mm_shuffle0000_ps(other1.values);
            const __m128 val1A = _mm_shuffle1111_ps(other1.values);
            const __m128 val2A = _mm_shuffle2222_ps(other1.values);
            const __m128 val0B = _mm_shuffle0000_ps(other2.values);
            const __m128 val1B = _mm_shuffle1111_ps(other2.values);
            const __m128 val2B = _mm_shuffle2222_ps(other2.values);
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                const __m512 val0 = _mm512_insertf32x4(_mm512_castps256_ps512(_mm256_set_m128(val1A, val0A)), val2A, 2);
                const __m512 val1 = _mm512_insertf32x4(_mm512_castps256_ps512(_mm256_set_m128(val1B, val0B)), val2B, 2);
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                ...
                const __m256 valA = _mm256_set_m128(val1A, val0A);
                const __m256 valB = _mm256_set_m128(val1B, val0B);
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                    return SIMD12(
                        _mm256_fmadd_ps(this->values0, valA, valB), _mm_fmadd_ps(this->values1, val2A, val2B));
                } else {
                    return SIMD12(_mm256_add_ps(_mm256_mul_ps(this->values0, valA), valB),
                        _mm_add_ps(_mm_mul_ps(this->values1, val2A), val2B));
                }
            } else {
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                    return SIMD12(_mm_fmadd_ps(this->values0, val0A, val0B), _mm_fmadd_ps(this->values1, val1A, val1B),
                        _mm_fmadd_ps(this->values2, val2A, val2B));
                } else {
                    return SIMD12(_mm_add_ps(_mm_mul_ps(this->values0, val0A), val0B),
                        _mm_add_ps(_mm_mul_ps(this->values1, val1A), val1B),
                        _mm_add_ps(_mm_mul_ps(this->values2, val2A), val2B));
                }
            }
        } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            const __m128i val0A = _mm_shuffle0000_epi32(other1.values);
            const __m128i val1A = _mm_shuffle1111_epi32(other1.values);
            const __m128i val2A = _mm_shuffle2222_epi32(other1.values);
            const __m128i val0B = _mm_shuffle0000_epi32(other2.values);
            const __m128i val1B = _mm_shuffle1111_epi32(other2.values);
            const __m128i val2B = _mm_shuffle2222_epi32(other2.values);
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_add_epi32(
                    _mm512_mullo_epi32(this->values,
                        _mm512_inserti32x4(_mm512_castsi256_si512(_mm256_set_m128i(val1A, val0A)), val2A, 2)),
                    _mm512_inserti32x4(_mm512_castsi256_si512(_mm256_set_m128i(val1B, val0B)), val2B, 2)));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_add_epi32(_mm256_mullo_epi32(this->values0, _mm256_set_m128i(val1A, val0A)),
                                  _mm256_set_m128i(val1B, val0B)),
                    _mm_add_epi32(_mm_mullo_epi32(this->values1, val2A), val2B));
            } else {
                return SIMD12(_mm_add_epi32(_mm_mullo_epi32(this->values0, val0A), val0B),
                    _mm_add_epi32(_mm_mullo_epi32(this->values1, val1A), val1B),
                    _mm_add_epi32(_mm_mullo_epi32(this->values2, val2A), val2B));
            }
        } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            const __m128i val0A =
                _mm_shuffle_epi8(other1.values, _mm_set_epi8(3, 2, 3, 2, 3, 2, 3, 2, 1, 0, 1, 0, 1, 0, 1, 0));
            const __m128i val1A = _mm_shufflelo_epi16(other1.values, _MM_SHUFFLE(2, 2, 2, 2));
            const __m128i val0B =
                _mm_shuffle_epi8(other2.values, _mm_set_epi8(3, 2, 3, 2, 3, 2, 3, 2, 1, 0, 1, 0, 1, 0, 1, 0));
            const __m128i val1B = _mm_shufflelo_epi16(other2.values, _MM_SHUFFLE(2, 2, 2, 2));
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_add_epi16(
                    _mm256_mullo_epi16(this->values, _mm256_set_m128i(val0A, val1A)), _mm256_set_m128i(val0B, val1B)));
            } else {
                return SIMD12(_mm_add_epi16(_mm_mullo_epi16(this->values0, val0A), val0B),
                    _mm_add_epi16(_mm_mullo_epi16(this->values1, val1A), val1B));
            }
        } else if constexpr (isSame<T, uint8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T>) {
                const __m256i ret = _mm256_add_epi16(
                    _mm256_mullo_epi16(_mm256_cvtepu8_epi16(this->values),
                        _mm256_cvtepu8_epi16(_mm_shuffle_epi8(
                            other1.values, _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0)))),
                    _mm256_cvtepu8_epi16(
                        _mm_shuffle_epi8(other2.values, _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0))));
                return SIMD12(_mm_packus_epi16(_mm256_castsi256_si128(ret), _mm256_extracti128_si256(ret, 1)));
            } else {
                const __m128i valA = _mm_cvtepu8_epi16(other1.values);
                const __m128i valB = _mm_cvtepu8_epi16(other2.values);
                const __m128i ret0 = _mm_add_epi16(
                    _mm_mullo_epi16(_mm_cvtepu8_epi16(this->values),
                        _mm_shuffle_epi8(valA, _mm_set_epi8(3, 2, 3, 2, 3, 2, 3, 2, 1, 0, 1, 0, 1, 0, 1, 0))),
                    _mm_shuffle_epi8(valB, _mm_set_epi8(3, 2, 3, 2, 3, 2, 3, 2, 1, 0, 1, 0, 1, 0, 1, 0)));
                const __m128i ret1 = _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepu8_epi16(_mm_bsrli_si128(this->values, 64)),
                                                       _mm_shufflelo_epi16(valA, _MM_SHUFFLE(2, 2, 2, 2))),
                    _mm_shufflelo_epi16(valB, _MM_SHUFFLE(2, 2, 2, 2)));
                return SIMD12(_mm_packus_epi16(ret0, ret1));
            }
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T>) {
                const __m256i ret = _mm256_add_epi16(
                    _mm256_mullo_epi16(_mm256_cvtepi8_epi16(this->values),
                        _mm256_cvtepi8_epi16(_mm_shuffle_epi8(
                            other1.values, _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0)))),
                    _mm256_cvtepi8_epi16(
                        _mm_shuffle_epi8(other2.values, _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0))));
                return SIMD12(_mm_packus_epi16(_mm256_castsi256_si128(ret), _mm256_extracti128_si256(ret, 1)));
            } else {
                const __m128i valA = _mm_cvtepi8_epi16(other1.values);
                const __m128i valB = _mm_cvtepi8_epi16(other2.values);
                const __m128i ret0 = _mm_add_epi16(
    ...
    XS_INLINE SIMD12 mad(const SIMD3Def& other1, const SIMD12& other2) const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                const __m512 val = _mm512_insertf32x4(_mm512_castps256_ps512(_mm256_set_m128(val1, val0)), val2, 2);
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                ...
                const __m256 val = _mm256_set_m128(val1, val0);
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                    return SIMD12(_mm256_fmadd_ps(this->values0, val, other2.values0),
                        _mm_fmadd_ps(this->values1, val2, other2.values1));
                } else {
                    return SIMD12(_mm256_add_ps(_mm256_mul_ps(this->values0, val), other2.values0),
                        _mm_add_ps(_mm_mul_ps(this->values1, val2), other2.values1));
                }
            } else {
                if constexpr (hasFMA<T> && (EvenIfNotFree || hasFMAFree<T>)) {
                    return SIMD12(_mm_fmadd_ps(this->values0, val0, other2.values0),
                        _mm_fmadd_ps(this->values1, val1, other2.values1),
                        _mm_fmadd_ps(this->values2, val2, other2.values2));
                } else {
                    return SIMD12(_mm_add_ps(_mm_mul_ps(this->values0, val0), other2.values0),
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_add_epi32(
                    _mm512_mullo_epi32(this->values,
                        _mm512_inserti32x4(_mm512_castsi256_si512(_mm256_set_m128i(val1, val0)), val2, 2)),
                    other2.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(
                    _mm256_add_epi32(_mm256_mullo_epi32(this->values0, _mm256_set_m128i(val1, val0)), other2.values0),
                    _mm_add_epi32(_mm_mullo_epi32(this->values1, val2), other2.values1));
            } else {
                return SIMD12(_mm_add_epi32(_mm_mullo_epi32(this->values0, val0), other2.values0),
            ...
            const __m128i val0 =
                _mm_shuffle_epi8(other1.values, _mm_set_epi8(3, 2, 3, 2, 3, 2, 3, 2, 1, 0, 1, 0, 1, 0, 1, 0));
            const __m128i val1 = _mm_shufflelo_epi16(other1.values, _MM_SHUFFLE(2, 2, 2, 2));
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(
                    _mm256_add_epi16(_mm256_mullo_epi16(this->values, _mm256_set_m128i(val0, val1)), other2.values));
            } else {
                return SIMD12(_mm_add_epi16(_mm_mullo_epi16(this->values0, val0), other2.values0),
                    _mm_add_epi16(_mm_mullo_epi16(this->values1, val1), other2.values1));
            }
        } else if constexpr (isSame<T, uint8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T>) {
                const __m256i ret =
                    _mm256_add_epi16(_mm256_mullo_epi16(_mm256_cvtepu8_epi16(this->values),
                                         _mm256_cvtepu8_epi16(_mm_shuffle_epi8(other1.values,
                ...
                const __m128i ret0 = _mm_add_epi16(
                    ...
                    _mm_cvtepu8_epi16(other2.values));
                const __m128i ret1 = _mm_add_epi16(_mm_mullo_epi16(_mm_cvtepu8_epi16(_mm_bsrli_si128(this->values, 64)),
                                                       _mm_shufflelo_epi16(val, _MM_SHUFFLE(2, 2, 2, 2))),
                    _mm_cvtepu8_epi16(_mm_bsrli_si128(other2.values, 64)));
                return SIMD12(_mm_packus_epi16(ret0, ret1));
            }
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T>) {
                const __m256i ret =
                    _mm256_add_epi16(_mm256_mullo_epi16(_mm256_cvtepi8_epi16(this->values),
                                         _mm256_cvtepi8_epi16(_mm_shuffle_epi8(other1.values,
                                             _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0)))),
                        _mm256_cvtepi8_epi16(other2.values));
                return SIMD12(_mm_packus_epi16(_mm256_castsi256_si128(ret), _mm256_extracti128_si256(ret, 1)));
            } else {
    ...
    XS_INLINE SIMD12 msub(const SIMD12& other1, const SIMD12& other2) const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_sub_epi32(_mm512_mullo_epi32(this->values, other1.values), other2.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_sub_epi32(_mm256_mullo_epi32(this->values0, other1.values0), other2.values0),
                    _mm_sub_epi32(_mm_mullo_epi32(this->values1, other1.values1), other2.values1));
            } else {
                return SIMD12(_mm_sub_epi32(_mm_mullo_epi32(this->values0, other1.values0), other2.values0),
                    _mm_sub_epi32(_mm_mullo_epi32(this->values1, other1.values1), other2.values1),
                    _mm_sub_epi32(_mm_mullo_epi32(this->values2, other1.values2), other2.values2));
            }
        } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_sub_epi16(_mm256_mullo_epi16(this->values, other1.values), other2.values));
            } else {
                return SIMD12(_mm_sub_epi16(_mm_mullo_epi16(this->values0, other1.values0), other2.values0),
    ...
    XS_INLINE SIMD12 abs() const noexcept
    {
        if constexpr (isUnsigned<T>) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_abs_epi32(this->values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_abs_epi32(this->values), _mm_abs_epi32(this->values1));
            } else {
                return SIMD12(_mm_abs_epi32(this->values0), _mm_abs_epi32(this->values1), _mm_abs_epi32(this->values2));
            }
        } else if constexpr (isSame<T, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_abs_epi16(this->values));
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_abs_epi8(this->values));
        }
#endif
        else {
    ...
    XS_INLINE SIMD12 max(const SIMD12& other) const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_max_epu32(this->values, other.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(
                    _mm256_max_epu32(this->values0, other.values0), _mm_max_epu32(this->values1, other.values1));
            } else {
                return SIMD12(_mm_max_epu32(this->values0, other.values0), _mm_max_epu32(this->values1, other.values1),
                    _mm_max_epu32(this->values2, other.values2));
            }
        } else if constexpr (isSame<T, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_max_epi32(this->values, other.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(
                    _mm256_max_epi32(this->values0, other.values0), _mm_max_epi32(this->values1, other.values1));
            } else {
                return SIMD12(_mm_max_epi32(this->values0, other.values0), _mm_max_epi32(this->values1, other.values1),
                    _mm_max_epi32(this->values2, other.values2));
            }
        } else if constexpr (isSame<T, uint16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_max_epu16(this->values, other.values));
            } else {
                return SIMD12(_mm_max_epu16(this->values0, other.values0), _mm_max_epu16(this->values1, other.values1));
            }
        } else if constexpr (isSame<T, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_max_epi16(this->values, other.values));
            } else {
                return SIMD12(_mm_max_epi16(this->values0, other.values0), _mm_max_epi16(this->values1, other.values1));
            }
        } else if constexpr (isSame<T, uint8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_max_epu8(this->values, other.values));
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_max_epi8(this->values, other.values));
        } else
#endif
        {
    ...
    XS_INLINE SIMD12 min(const SIMD12& other) const noexcept
    {
        ...
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_min_epu32(this->values, other.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(
                    _mm256_min_epu32(this->values0, other.values0), _mm_min_epu32(this->values1, other.values1));
            } else {
                return SIMD12(_mm_min_epu32(this->values0, other.values0), _mm_min_epu32(this->values1, other.values1),
                    _mm_min_epu32(this->values2, other.values2));
            }
        } else if constexpr (isSame<T, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_min_epi32(this->values, other.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(
                    _mm256_min_epi32(this->values0, other.values0), _mm_min_epi32(this->values1, other.values1));
            } else {
                return SIMD12(_mm_min_epi32(this->values0, other.values0), _mm_min_epi32(this->values1, other.values1),
                    _mm_min_epi32(this->values2, other.values2));
            }
        } else if constexpr (isSame<T, uint16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_min_epu16(this->values, other.values));
            } else {
                return SIMD12(_mm_min_epu16(this->values0, other.values0), _mm_min_epu16(this->values1, other.values1));
            }
        } else if constexpr (isSame<T, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_min_epi16(this->values, other.values));
            } else {
                return SIMD12(_mm_min_epi16(this->values0, other.values0), _mm_min_epi16(this->values1, other.values1));
            }
        } else if constexpr (isSame<T, uint8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_min_epu8(this->values, other.values));
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_min_epi8(this->values, other.values));
        } else
#endif
        {
     ...
     * @returns The maximum value.
     */
    XS_INLINE SIMD12 max(const BaseDef other) const noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                return SIMD12(_mm256_max_ps(this->values0, other.values), _mm_max_ps(this->values1, other.values));
            } else {
                return SIMD12(_mm_max_ps(this->values0, other.values0), _mm_max_ps(this->values1, other.values1),
                    _mm_max_ps(this->values2, other.values2));
            }
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                return SIMD12(_mm256_max_epu32(this->values0, other.values),
                    _mm_max_epu32(this->values1, _mm256_castsi256_si128(other.values)));
            } else {
                return SIMD12(_mm_max_epu32(this->values0, other.values), _mm_max_epu32(this->values1, other.values),
                    _mm_max_epu32(this->values2, other.values));
            }
        } else if constexpr (isSame<T, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_max_epi32(this->values, other.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_max_epi32(this->values0, other.values),
                    _mm_max_epi32(this->values1, _mm256_castsi256_si128(other.values)));
            } else {
                return SIMD12(_mm_max_epi32(this->values0, other.values), _mm_max_epi32(this->values1, other.values),
                    _mm_max_epi32(this->values2, other.values));
            }
        } else if constexpr (isSame<T, uint16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_max_epu16(this->values, other.values));
            } else {
                return SIMD12(_mm_max_epu16(this->values0, other.values), _mm_max_epu16(this->values1, other.values));
            }
        } else if constexpr (isSame<T, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_max_epi16(this->values, other.values));
            } else {
                return SIMD12(_mm_max_epi16(this->values0, other.values), _mm_max_epi16(this->values1, other.values));
            }
        } else if constexpr (isSame<T, uint8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_max_epu8(this->values, other.values));
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
     ...
     * @returns The minimum value.
     */
    XS_INLINE SIMD12 min(const BaseDef other) const noexcept
    {
#if XS_ISA == XS_X86
        if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                return SIMD12(_mm256_min_ps(this->values0, other.values), _mm_min_ps(this->values1, other.values));
            } else {
                return SIMD12(_mm_min_ps(this->values0, other.values0), _mm_min_ps(this->values1, other.values1),
                    _mm_min_ps(this->values2, other.values2));
            }
            ...
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                ...
                return SIMD12(_mm256_min_epu32(this->values0, other.values),
                    _mm_min_epu32(this->values1, _mm256_castsi256_si128(other.values)));
            } else {
                return SIMD12(_mm_min_epu32(this->values0, other.values), _mm_min_epu32(this->values1, other.values),
                    _mm_min_epu32(this->values2, other.values));
            }
        } else if constexpr (isSame<T, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
                return SIMD12(_mm512_min_epi32(this->values, other.values));
            } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_min_epi32(this->values0, other.values),
                    _mm_min_epi32(this->values1, _mm256_castsi256_si128(other.values)));
            } else {
                return SIMD12(_mm_min_epi32(this->values0, other.values), _mm_min_epi32(this->values1, other.values),
                    _mm_min_epi32(this->values2, other.values));
            }
        } else if constexpr (isSame<T, uint16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_min_epu16(this->values, other.values));
            } else {
                return SIMD12(_mm_min_epu16(this->values0, other.values), _mm_min_epu16(this->values1, other.values));
            }
        } else if constexpr (isSame<T, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
                return SIMD12(_mm256_min_epi16(this->values, other.values));
            } else {
                return SIMD12(_mm_min_epi16(this->values0, other.values), _mm_min_epi16(this->values1, other.values));
            }
        } else if constexpr (isSame<T, uint8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
            return SIMD12(_mm_min_epu8(this->values, other.values));
        } else if constexpr (isSame<T, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
...
XS_INLINE SIMD12<T, Width> operator+(const SIMD12<T, Width>& other1, const SIMD12<T, Width>& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm_add_ps(other1.values0, other2.values0),
                _mm_add_ps(other1.values1, other2.values1), _mm_add_ps(other1.values2, other2.values2));
        }
    } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            return SIMD12<T, Width>(_mm512_add_epi32(other1.values, other2.values));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(
                _mm256_add_epi32(other1.values0, other2.values0), _mm_add_epi32(other1.values1, other2.values1));
        } else {
            return SIMD12<T, Width>(_mm_add_epi32(other1.values0, other2.values0),
                _mm_add_epi32(other1.values1, other2.values1), _mm_add_epi32(other1.values2, other2.values2));
        }
    } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_add_epi16(other1.values, other2.values));
        } else {
            return SIMD12<T, Width>(
                _mm_add_epi16(other1.values0, other2.values0), _mm_add_epi16(other1.values1, other2.values1));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_add_epi8(other1.values, other2.values));
    } else
#endif
    {
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::BaseDef other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm_add_ps(other1.values0, other2.values),
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm256_add_epi32(other1.values0, other2.values),
                _mm_add_epi32(other1.values1, _mm256_castsi256_si128(other2.values)));
        } else {
            return SIMD12<T, Width>(_mm_add_epi32(other1.values0, other2.values),
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::SIMD2Def& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            return SIMD12<T, Width>(_mm512_add_epi32(other1.values, _mm512_broadcasti64(other2.values)));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_add_epi32(other1.values0, _mm256_broadcasti64(other2.values)),
                _mm_add_epi32(other1.values1, other2.values));
        } else {
            const __m128 val = _mm_shuffle1010_epi32(other2.values);
            return SIMD12<T, Width>(_mm_add_epi32(other1.values0, val), _mm_add_epi32(other1.values1, val),
                _mm_add_epi32(other1.values2, val));
        }
    } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_add_epi16(other1.values, _mm256_broadcasti32(other2.values)));
        } else {
            const __m128 val0 = _mm_shuffle0000_epi32(other2.values);
            return SIMD12<T, Width>(_mm_add_epi16(other1.values0, val0), _mm_add_epi16(other1.values1, val0));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_add_epi8(other1.values, _mm_broadcastw_epi16(other2.values)));
    } else
#endif
    {
        return SIMD12<T, Width>(other1.values0 + other2.values0, other1.values1 + other2.values1,
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::SIMD3Def& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        ...
        const __m128 val2 = _mm_shuffle2222_ps(other2.values);
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            const __m512 val = _mm512_insertf32x4(
                _mm512_castps256_ps512(_mm256_set_m128(val1, val0)), _mm_shuffle2222_ps(other2.values), 2);
            return SIMD12<T, Width>(_mm512_add_ps(other1.values, val));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            const __m256 val = _mm256_set_m128(val1, val0);
            return SIMD12<T, Width>(_mm256_add_ps(other1.values0, val), _mm_add_ps(other1.values1, val2));
        } else {
            return SIMD12<T, Width>(
                _mm_add_ps(other1.values0, val0), _mm_add_ps(other1.values1, val1), _mm_add_ps(other1.values2, val2));
        }
    } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        const __m128i val0 = _mm_shuffle0000_epi32(other2.values);
        const __m128i val1 = _mm_shuffle1111_epi32(other2.values);
        const __m128i val2 = _mm_shuffle2222_epi32(other2.values);
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            const __m512i val = _mm512_inserti32x4(_mm512_castsi256_si512(_mm256_set_m128i(val1, val0)), val2, 2);
            return SIMD12<T, Width>(_mm512_add_epi32(other1.values, val));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            const __m256i val = _mm256_set_m128i(val1, val0);
            return SIMD12<T, Width>(_mm256_add_epi32(other1.values0, val), _mm_add_epi32(other1.values1, val2));
        } else {
            return SIMD12<T, Width>(_mm_add_epi32(other1.values0, val0), _mm_add_epi32(other1.values1, val1),
                _mm_add_epi32(other1.values2, val2));
        }
    } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        const __m128i val0 =
            _mm_shuffle_epi8(other2.values, _mm_set_epi8(3, 2, 3, 2, 3, 2, 3, 2, 1, 0, 1, 0, 1, 0, 1, 0));
        const __m128i val1 = _mm_shufflelo_epi16(other2.values, _MM_SHUFFLE(2, 2, 2, 2));
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD3x3<T, Width>(_mm256_add_epi16(other1.values, _mm256_set_m128i(val1, val0)));
        } else {
            return SIMD3x3<T, Width>(_mm_add_epi16(other1.values0, val0), _mm_add_epi16(other1.values1, val1));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD3x3<T, Width>(_mm_add_epi8(other1.values,
            _mm_shuffle_epi8(other2.values, _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0))));
    } else
#endif
    {
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::SIMD4Def& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            return SIMD12<T, Width>(_mm512_add_api32(other1.values, _mm512_broadcastsi128(other2.values)));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_add_epi32(other1.values0, _mm256_broadcasti128(other2.values)),
                _mm_add_epi32(other1.values1, other2.values));
        } else {
            return SIMD12<T, Width>(_mm_add_epi32(other1.values0, other2.values),
                _mm_add_epi32(other1.values1, other2.values), _mm_add_epi32(other1.values2, other2.values));
        ...
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_add_epi16(other1.values, _mm256_broadcasti64(other2.values)));
        } else {
            return SIMD12<T, Width>(_mm_add_epi16(other1.values0, _mm_shuffle1010_epi32(other2.values)),
                _mm_add_epi16(other1.values1, other2.values));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_add_epi8(other1.values, _mm_shuffle0000_epi32(other2.values)));
    } else
#endif
    {
        return SIMD12<T, Width>(other1.values0 + other2.values0, other1.values1 + other2.values1,
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::SIMD6Def& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            const __m128 val0 = _mm_shuffle1010_ps(other2.values0);
            const __m128 val1 = _mm_shuffle3232_ps(other2.values0);
            const __m128 val2 = _mm_shuffle1010_ps(other2.values1);
            return SIMD12<T, Width>(
                _mm_add_ps(other1.values0, val0), _mm_add_ps(other1.values1, val1), _mm_add_ps(other1.values2, val2));
        }
    } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            const __m256i val = _mm256_shuffle1010_epi32(other2.values);
            __m512i val2 = _mm512_set_m256i(
                _mm256_castsi128_si256(_mm_shuffle3232_epi32(_mm256_castsi256_si128(other2.values))), val);
            val2 = _mm512_shuffle_i32x4(val2, val2, _MM_SHUFFLE(1, 1, 2, 0));
            return SIMD12<T, Width>(_mm512_add_epi32(other1.values, val2));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            const __m256i val = _mm256_shuffle1010_epi32(other2.values);
            const __m256i val0 =
                _mm256_insertf128_epi32(val, _mm_shuffle3232_epi32(_mm256_castsi256_si128(other2.values)), 1);
            const __m128i val1 = _mm256_extracti128_si256(val, 1);
            return SIMD12<T, Width>(_mm256_add_epi32(other1.values0, val0), _mm_add_epi32(other1.values1, val1));
        } else {
            const __m128i val0 = _mm_shuffle1010_epi32(other2.values0);
            const __m128i val1 = _mm_shuffle3232_epi32(other2.values0);
            const __m128i val2 = _mm_shuffle1010_epi32(other2.values1);
            return SIMD12<T, Width>(_mm_add_epi32(other1.values0, val0), _mm_add_epi32(other1.values1, val1),
                _mm_add_epi32(other1.values2, val2));
        }
    } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_add_epi16(other1.values,
                _mm256_set_m128i(_mm_shuffle76765454_epi16(other2.values), _mm_shuffle32321010_epi16(other2.values))));
        } else {
            return SIMD12<T, Width>(_mm_add_epi16(other1.values0, _mm_shuffle32321010_epi16(other2.values)),
                _mm_add_epi16(other1.values1, _mm_shuffle76765454_epi16(other2.values)));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_add_epi8(other1.values,
...
XS_INLINE SIMD12<T, Width> operator-(const SIMD12<T, Width>& other1, const SIMD12<T, Width>& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm_sub_ps(other1.values0, other2.values0),
                _mm_sub_ps(other1.values1, other2.values1), _mm_sub_ps(other1.values2, other2.values2));
        }
    } else if constexpr (isSameAny<T, uint32, int32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            return SIMD12<T, Width>(_mm512_sub_epi32(other1.values, other2.values));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(
                _mm256_sub_epi32(other1.values0, other2.values0), _mm_sub_epi32(other1.values1, other2.values1));
        } else {
            return SIMD12<T, Width>(_mm_sub_epi32(other1.values0, other2.values0),
                _mm_sub_epi32(other1.values1, other2.values1), _mm_sub_epi32(other1.values2, other2.values2));
        }
    } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_sub_epi16(other1.values, other2.values));
        } else {
            return SIMD12<T, Width>(
                _mm_sub_epi16(other1.values0, other2.values0), _mm_sub_epi16(other1.values1, other2.values1));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_sub_epi8(other1.values, other2.values));
    } else
#endif
    {
        return SIMD12<T, Width>(other1.values0 - other2.values0, other1.values1 - other2.values1,
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::BaseDef other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm_sub_ps(other1.values0, other2.values),
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm256_sub_epi32(other1.values0, other2.values),
                _mm_sub_epi32(other1.values1, _mm256_castsi256_si128(other2.values)));
        } else {
            return SIMD12<T, Width>(_mm_sub_epi32(other1.values0, other2.values),
    ...
    const typename SIMD12<T, Width>::BaseDef other1, const SIMD12<T, Width>& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm_sub_ps(other1.values, other2.values0),
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            return SIMD12<T, Width>(_mm256_sub_epi32(other1.values, other2.values0),
                _mm_sub_epi32(_mm256_castsi256_si128(other1.values), other2.values1));
        } else {
            return SIMD12<T, Width>(_mm_sub_epi32(other1.values, other2.values0),
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::SIMD2Def& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            return SIMD12<T, Width>(_mm512_sub_epi32(other1.values, _mm512_broadcasti64(other2.values)));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_sub_epi32(other1.values0, _mm256_broadcasti64(other2.values)),
                _mm_sub_epi32(other1.values1, other2.values));
        } else {
            const __m128 val = _mm_shuffle1010_epi32(other2.values);
            return SIMD12<T, Width>(_mm_sub_epi32(other1.values0, val), _mm_sub_epi32(other1.values1, val),
                _mm_sub_epi32(other1.values2, val));
        }
    } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_sub_epi16(other1.values, _mm256_broadcasti32(other2.values)));
        } else {
            const __m128 val0 = _mm_shuffle0000_epi32(other2.values);
            return SIMD12<T, Width>(_mm_sub_epi16(other1.values0, val0), _mm_sub_epi16(other1.values1, val0));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_sub_epi8(other1.values, _mm_broadcastw_epi16(other2.values)));
    } else
#endif
    {
        return SIMD12<T, Width>(other1.values0 - other2.values0, other1.values1 - other2.values1,
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::SIMD3Def& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            const __m256 val = _mm256_set_m128(val1, val0);
            return SIMD12<T, Width>(_mm256_sub_ps(other1.values0, val), _mm_sub_ps(other1.values1, val2));
        } else {
            return SIMD12<T, Width>(
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            const __m512i val = _mm512_inserti32x4(_mm512_castsi256_si512(_mm256_set_m128i(val1, val0)), val2, 2);
            return SIMD12<T, Width>(_mm512_sub_epi32(other1.values, val));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            const __m256i val = _mm256_set_m128i(val1, val0);
            return SIMD12<T, Width>(_mm256_sub_epi32(other1.values0, val), _mm_sub_epi32(other1.values1, val2));
        } else {
            return SIMD12<T, Width>(_mm_sub_epi32(other1.values0, val0), _mm_sub_epi32(other1.values1, val1),
                _mm_sub_epi32(other1.values2, val2));
        }
    } else if constexpr (isSameAny<T, uint16, int16> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        const __m128i val0 =
            _mm_shuffle_epi8(other2.values, _mm_set_epi8(3, 2, 3, 2, 3, 2, 3, 2, 1, 0, 1, 0, 1, 0, 1, 0));
        const __m128i val1 = _mm_shufflelo_epi16(other2.values, _MM_SHUFFLE(2, 2, 2, 2));
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD3x3<T, Width>(_mm256_sub_epi16(other1.values, _mm256_set_m128i(val1, val0)));
        } else {
            return SIMD3x3<T, Width>(_mm_sub_epi16(other1.values0, val0), _mm_sub_epi16(other1.values1, val1));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD3x3<T, Width>(_mm_sub_epi8(other1.values,
            _mm_shuffle_epi8(other2.values, _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0))));
    } else
#endif
    {
    ...
    const SIMD12<T, Width>& other1, const typename SIMD12<T, Width>::SIMD4Def& other2) noexcept
{
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            return SIMD12<T, Width>(_mm512_sub_api32(other1.values, _mm512_broadcastsi128(other2.values)));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_sub_epi32(other1.values0, _mm256_broadcasti128(other2.values)),
                _mm_sub_epi32(other1.values1, other2.values));
        } else {
            return SIMD12<T, Width>(_mm_sub_epi32(other1.values0, other2.values),
                _mm_sub_epi32(other1.values1, other2.values), _mm_sub_epi32(other1.values2, other2.values));
        ...
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_sub_epi16(other1.values, _mm256_broadcasti64(other2.values)));
        } else {
            return SIMD12<T, Width>(_mm_sub_epi16(other1.values0, _mm_shuffle1010_epi32(other2.values)),
                _mm_sub_epi16(other1.values1, other2.values));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_sub_epi8(other1.values, _mm_shuffle0000_epi32(other2.values)));
    } else
#endif
    {
        return SIMD12<T, Width>(other1.values0 - other2.values0, other1.values1 - other2.values1,
    ...
    if constexpr (isSame<T, float32> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        ...
        if constexpr (hasSIMD512<T> && (Width >= SIMDWidth::B64)) {
            ...
            __m512i val2 = _mm512_set_m256i(
                _mm256_castsi128_si256(_mm_shuffle3232_epi32(_mm256_castsi256_si128(other2.values))), val);
            val2 = _mm512_shuffle_i32x4(val2, val2, _MM_SHUFFLE(1, 1, 2, 0));
            return SIMD12<T, Width>(_mm512_sub_epi32(other1.values, val2));
        } else if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            const __m256i val = _mm256_shuffle1010_epi32(other2.values);
            const __m256i val0 =
                _mm256_insertf128_epi32(val, _mm_shuffle3232_epi32(_mm256_castsi256_si128(other2.values)), 1);
            const __m128i val1 = _mm256_extracti128_si256(val, 1);
            return SIMD12<T, Width>(_mm256_sub_epi32(other1.values0, val0), _mm_sub_epi32(other1.values1, val1));
        } else {
            const __m128i val0 = _mm_shuffle1010_epi32(other2.values0);
            const __m128i val1 = _mm_shuffle3232_epi32(other2.values0);
            const __m128i val2 = _mm_shuffle1010_epi32(other2.values1);
        ...
        if constexpr (hasSIMD256<T> && (Width >= SIMDWidth::B32)) {
            return SIMD12<T, Width>(_mm256_sub_epi16(other1.values,
                _mm256_set_m128i(_mm_shuffle76765454_epi16(other2.values), _mm_shuffle32321010_epi16(other2.values))));
        } else {
            return SIMD12<T, Width>(_mm_sub_epi16(other1.values0, _mm_shuffle32321010_epi16(other2.values)),
                _mm_sub_epi16(other1.values1, _mm_shuffle76765454_epi16(other2.values)));
        }
    } else if constexpr (isSameAny<T, uint8, int8> && hasSIMD<T> && (Width > SIMDWidth::Scalar)) {
        return SIMD12<T, Width>(_mm_sub_epi8(other1.values,
            _mm_shuffle_epi8(other2.values, _mm_set_epi8(7, 6, 7, 6, 5, 4, 5, 4, 3, 2, 3, 2, 1, 0, 1, 0))));
    } else
#endif
    {
