...
template<typename T>
class PArrayTest : public ::testing::Test
{
public:
    using Type = T;
};
...
TYPED_TEST_SUITE(PArrayTest, MemoryTestTypes, MemoryTestNames);

TYPED_TEST_NS2(PArray, PArrayTest, Constructor)
{
    using TestType = typename TestFixture::Type;

    auto test1 = PArray<TestType>(8);
    ...
    PArray<TestType> test3;
    ASSERT_FALSE(test3.isValid());

    PArray<TestType> test4(test1);
    ASSERT_EQ(test4.getSize(), test1.getSize());
}

TYPED_TEST_NS2(PArray, PArrayTest, AddElement)
{
    using TestType = typename TestFixture::Type;
    auto test1 = PArray<TestType>(8);
    ASSERT_TRUE(test1.isValid());
