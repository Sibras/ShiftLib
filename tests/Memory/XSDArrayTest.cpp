...
TYPED_TEST_SUITE(DArrayTest, MemoryTestTypes, MemoryTestNames);

TYPED_TEST_NS2(DArray, DArrayTest, Constructor)
{
    using TestType = typename TestFixture::Type;

    auto test1 = DArray<TestType>(8);
    ...
    DArray<TestType> test3;
    ASSERT_FALSE(test3.isValid());

    DArray<TestType> test4(test1);
    ASSERT_EQ(test4.getSize(), test1.getSize());
}

TYPED_TEST_NS2(DArray, DArrayTest, AddElement)
{
    using TestType = typename TestFixture::Type;
    auto test1 = DArray<TestType>(8);
    ASSERT_TRUE(test1.isValid());
